...
auto handle_options(int argc, char* argv[]){
  ...
  try
  {
    ...
    ("a", bpo::value<double>()->required(), "Gaussian dispersal parameter")
...
public:
  static auto run(bpo::variables_map const& vm)
  {
    ...
    *****************************/
    using demographic_policy = quetzal::demography::strategy::individual_based;
    ...
    ******************/
    ...
    using kernel_type = demo::dispersal::Gaussian;
    double a = vm["a"].as<double>();
    kernel_type::param_type param(a);
    auto f = [param, distance](coord_type const& x, coord_type const& y){
      return kernel_type::pdf(distance(x,y), param);
    };
    auto dispersal = demographic_policy::make_distance_based_dispersal(space, f);
